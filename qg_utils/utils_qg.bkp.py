...
def convert_examples_to_gen_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        # The -4 accounts for <bop>,<eop>(paragraph) and <eop>(phrase) and <eop>(question)
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in range(doc_span.length):
                ...
                segment_ids.append(seg_a)
            tokens.append(eop_token) #"[MASK]")
            segment_ids.append(seg_b) 
            ...
            segment_ids = segment_ids + [ seg_b for i in range(tok_end_position-tok_start_position+1) ]
            segment_ids.append(seg_b) 
