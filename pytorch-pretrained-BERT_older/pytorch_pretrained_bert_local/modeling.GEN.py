...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.alter_tok_embeddings is not None:
          ...
          self.token_2_type_embeddings = nn.Embedding(2, config.hidden_size)

          #share weights from self.token_type_embeddings
          #self.embeddings4[:2,:] = self.token_type_embeddings.weight
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        if self.alter_tok_embeddings is None:
          ...
          #change to one hot vector - remove
          #token_type_ids_1hot = torch.zeros_like(token_type_ids.unsqueeze(-1)).expand(-1,-1,2).float()
          #type_token_type_ids_1hot = token_type_ids_1hot.scatter(2,token_type_ids.unsqueeze(-1),1)
          #token_type_embeddings = token_type_ids_1hot.matmul(self.token_type_embeddings.weight) #b x seq x 4  *  . x 4 x hidden_size
          ...
          mat=torch.cat((self.token_type_embeddings.weight, self.token_2_type_embeddings.weight),0)
          #print(mat.size())
          #print(token_type_ids.size(), token_type_ids.sum(1))
          #input("check")
          #token_type_embeddings = token_type_ids.matmul(self.token_4_type_embeddings.weight) #b x seq x 4  *  . x 4 x hidden_size
          token_type_embeddings = token_type_ids.matmul(mat) # b x seq x 4  *  . x 4 x hidden_size
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.sigma_linear = nn.Linear(config.hidden_size, config.hidden_size) 

    def reparameterize(self, hidden_states):
        mu = self.mu_linear(hidden_states[:,0]) #Only [CLS] token to be latent
        logvar = self.mu_sigma(hidden_states[:,0]) #Only [CLS] token to be latent
        std = torch.exp(0.5*logvar)
        eps = torch.randn_like(std)
        latent_cls = mu + eps*std
        new_hidden_states = torch.cat((latent_cls, hidden_states[:,1:]),0)

        return new_hidden_states
        

    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, attn=False,latent_layer=-1):
        all_encoder_layers = []
        all_attentions = []
        lno=0
        for layer_module in self.layer:
            ...
            if lno == latent_layer: 
                hidden_states = self.reparameterize(hidden_states)
            lno += 1
...
class BertForQuestionGenerationFluidSegments(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.attention_over_attention = nn.Linear(config.hidden_size, 3 ) # config.n_heads * config.n_layers
    ...
    def forward(self, input_ids, token_classes, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):

        print("Forward for Question Generation -- 1863")
        ...
        answer_phrase_ids = answer_phrase_ids.float().matmul(torch.triu(torch.ones(answer_phrase_ids.size(1), answer_phrase_ids.size(1)) ).cuda() ).long() * 1 # segment_id 1 for answer phrase
        ...
        sequence_output, pooled_output, attns = self.bert(input_ids, token_classes, extra_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
...
class BertForQuestionGenerationFSeg(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(BertForQuestionGenerationFSeg, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
    ...
    def get_copy_probs_lateral(self,sequence_output, self_attentions,src_seq,hop=2): # This is 2-D Attention
        ...
        if hop == 3: # x*a + y* a^2 + z*a^3
          ...
          total_attention = torch.bmm(attention, attention) #attention_square
        elif hop == 21:
          ...
          total_attention = torch.bmm(attention_lateral, attention)#attention_square
        ...
        # copy_probs = copy_probs / copy_probs.sum(-1).unsqueeze(-1) # pa neccessaire
        return copy_probs
    def get_copy_probs(self,sequence_output, self_attentions,src_seq,cls_att=0,answer_ids=None,maxout_pointer=False):
    ...
    def forward(self, input_ids, token_classes, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        print("Forward for Question Generation -- 2205")
        # Get Decoder Mask for Question Part
        #attention_mask = attention_mask.unsqueeze(1).expand(-1,attention_mask.size(-1),-1) # Use batch_size x seq_length x seq_length instead of batch_size x seq_length
...
class BertForQuestionGeneration(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        #question_mask_ids = token_type_ids.unsqueeze(-1).float()
        #question_mask_ids = (token_type_ids == 1).unsqueeze(-1).float()
        question_mask_ids = (token_type_ids == 4).unsqueeze(-1).float()
        input_mask_ids = (input_ids!=0).unsqueeze(-1).float()
        ...
        if copymethod > 0:
          ...
          #print("no copy method")
          #qgen_logits =  self.qgen_outputs(sequence_output)
          qgen_logits = torch.softmax(self.qgen_outputs(sequence_output),dim=-1)
...
class BertEncoder2Decoder(BertPreTrainedModel):
    ...
    def forward(self, input_ids, output_ids, input_token_type_ids=None, output_token_type_ids=None, input_mask=None, output_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False,copymethod=1,use_cls_only=True):



        #Encode the input
        answer_phrase_ids = torch.zeros_like(input_ids).long()
        '''
        answer_phrase_ids = torch.zeros_like(input_ids)
        answer_phrase_ids = answer_phrase_ids.scatter(-1,start_positions.unsqueeze(-1),1)
        answer_phrase_ids = answer_phrase_ids.scatter(-1,(end_positions+1).unsqueeze(-1),-1)
        ...
        #encoder_output, pooled_encoder_output, self_attns = self.bert_encoder(input_ids, input_token_type_ids + answer_phrase_ids, input_mask, 
        #        output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        encoder_output, pooled_encoder_output, self_attns = self.bert_encoder(input_ids, input_token_type_ids , input_mask, 
                output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)

        #Use just cls:
        if use_cls_only:
           encoder_output = encoder_output[:,0:1,:]
           input_mask_ids = input_mask_ids[:,0:1]
        ...
        output_mask = output_mask * torch.tril(torch.ones_like(output_mask[0]))

        prediction_ids = output_ids
        #prediction_ids[:,0]=self.PADID ## This causes change in in-line position; which can't be back-propogated
                                        ## Is this necessary? because we are already left shifting one time step (cyclically) below
        prediction_ids = torch.cat((prediction_ids[:,1:],prediction_ids[:,0:1]),-1) # batch_size x seq_length
        ...
        if copymethod > 0:
          ...
          qgen_logits = p * self.qgen_outputs(decoder_output)
          ...
          #print("no copy method")
          qgen_logits =  self.qgen_outputs(decoder_output)
