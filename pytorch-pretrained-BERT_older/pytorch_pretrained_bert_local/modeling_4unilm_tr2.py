...
class BertForQuestionGeneration(BertPreTrainedModel):
    ...
    def get_copy_probs(self,sequence_output, self_attentions,src_seq,cls_att=0,answer_ids=None,maxout_pointer=False):
        ...
        # attention value doesn't sum up to 1? why? - because of dropout?
        # src_seq has the source vocab ids
        copy_probs=torch.zeros(src_seq.size(0), src_seq.size(1), self.config.vocab_size)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        # For Question Generation use below line
        prediction_ids = (input_ids.unsqueeze(-1) * question_mask_ids.long()).squeeze(-1)
        input_ids[:,-1].fill_(self.MASKID) # last word as MASK 

        '''
        random_prediction_ids = prediction_ids.new(prediction_ids.size()).random_(0,10)
        #mask_ids = input_ids.new(input_ids.size(0), 1).fill_(MASK_ID)
        perten=5
        random_prediction_ids = question_mask_ids.long()*(random_prediction_ids.unsqueeze(-1) <=(perten-1)).long() #70% [MASK] probability
        random_prediction_ids = random_prediction_ids.squeeze(-1)

        prediction_ids = prediction_ids * random_prediction_ids

        # NOT MASKING INPUT WITH BELOW LINE
        input_ids = input_ids.masked_fill(random_prediction_ids.byte(),self.MASKID) 
        '''
        ...
        #print
        para_mask = torch.bmm((1-question_mask_ids)*input_mask_ids, ((1-question_mask_ids)*input_mask_ids).transpose(1,2))
        #para_mask = torch.bmm(question_mask_ids*input_mask_ids, (question_mask_ids*input_mask_ids).transpose(1,2))
        question2para_mask = torch.bmm(input_mask_ids, ((1-question_mask_ids)*input_mask_ids).transpose(1,2))
        #para_mask : should be a block where para is present 
        ...
        token_type_ids = token_type_ids + 4

        '''
        print(token_type_ids[0])
        print("#gen "," ".join(tokenizer.convert_ids_to_tokens([ input_ids[0][i].item() for i in range(input_ids[0].size(0)) if token_type_ids[0][i].item() ==5] )))
        input("token ids")
        print(extra_mask[0])
        input("extra mask")
        '''
        ...
        #sequence_output, pooled_output, attns = self.bert(input_ids, token_type_ids + answer_phrase_ids, extra_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        #sequence_output, pooled_output, attns = self.bert(input_ids, token_type_ids , extra_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        sequence_output, pooled_output, attns = self.bert(input_ids, token_type_ids , input_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        ...
        if copymethod > 0:
          ...
          qgen_logits = p*torch.softmax(qgen_logits,dim=-1)
