...
echo -dmS ${screen_name}
echo -S ${screen_name} -X stuff 'export\n'
echo -S ${screen_name} -X stuff 'source activate clone\n'
eval echo -S ${screen_name} -X stuff 'CUDA_VISIBLE_DEVICES=$cuda python qg_examples/run_qg.py  --output_dir $output_dir  --train_file data/squad/train-v1.1.json --predict_file data/squad_unilm/squad.unilm.test.json --predict_batch_size=1 --train_batch_size=6 --gradient_accumulation_steps 6 --max_seq_length=384 --doc_stride=128 --bert_model $output_dir --copymethod=1 --do_predict --data_cache_name $model --model_type $modeltype  --genfrom $genfrom --gento $gento --tag $cuda\n'


#echo -S $model"-test-"$cuda"-"$dash -X stuff CUDA_VISIBLE_DEVICES=$cuda python qg_examples/run_qg_eval.py  --output_dir $output_dir  --train_file data/squad/train-v1.1.json --predict_file data/squad_unilm/squad.unilm.test.json --predict_batch_size=1 --train_batch_size=6 --gradient_accumulation_steps 6 --learning_rate=3e-5 --num_train_epochs=3.0 --max_seq_length=384 --doc_stride=128 --bert_model $output_dir --copymethod=1  --do_predict --data_cache_name $model --model_type $modeltype --tok_model ${bert_models}/$savedmodel $extra --genfrom $genfrom --gento $gento --tag $cuda"-"$dash\n
...
echo -S ${screen_name} -X stuff 'export\n' 
echo -S ${screen_name} -X stuff 'source activate clone\n'
eval echo -S ${screen_name} -X stuff 'CUDA_VISIBLE_DEVICES=$last_cuda python qg_examples/run_qg.py  --output_dir $output_dir  --train_file data/squad/train-v1.1.json --predict_file data/squad_unilm/squad.unilm.test.json --predict_batch_size=1 --train_batch_size=6 --gradient_accumulation_steps 6 --max_seq_length=384 --doc_stride=128 --bert_model $output_dir --copymethod=1 --do_predict --data_cache_name $model --model_type $modeltype  --genfrom $genfrom --gento $gento --tag $last_cuda\n'
