...
CUDA_LAUNCH_BLOCKING=1 $python qg_examples/run_qg_actualviz.py  --output_dir $output_dir  --train_file data/squad_unilm/squad.unilm.train.json --predict_file data/squad_unilm/squad.unilm.dev.json --predict_batch_size=${predict_batch_size} --train_batch_size=8  --learning_rate=$learning_rate --num_train_epochs=5.0 --max_seq_length=384 --doc_stride=128 --bert_model ${bert_models}/$savedmodel --copymethod=1 --training_sanity_check_steps $evalsteps  --data_cache_name $model --testset --model_type $modeltype $genduring $*
