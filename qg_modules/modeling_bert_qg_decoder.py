...
class BertForQuestionGeneration(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.question_encoder = BertModel(config)
        self.boq_id = 103
    ...
    def forward(self, input_ids, 
        ...
        SEP_INDICES = [i for i, x in enumerate(input_ids[0]) if x == self.boq_id]
        #question_ids = input_ids[0][CLS_INDICES[0]+1:SEP_INDICES[0]]

        #Mask begin of question
        boq_mask = (input_ids != self.boq_id).unsqueeze(-1).float()
        #question_start = (input_ids == self.boq_id).float().unsqueeze(-1) # b x seq x 1
        question_start = torch.zeros_like(input_ids).float()
        question_start[:, SEP_INDICES[0]] = 1 # works only for batch_size 1
        ...
        if question_emb is None and query_ids is not None:
          q_outputs = self.question_encoder(query_ids)#
          question_emb = q_outputs["last_hidden_state"][:,0:1,:] # b x 1 x hidden_size
          question_emb = (question_emb*question_start) # b x seq x 1 * bx1xhidden_size
        elif question_emb is None:
            raise ValueError("You cannot specify both query_ids and question_emb None at the same time")
        ...
        # Adding Question Embedding to QUERY_START token [MASK]
        inputs_embeds = self.bert.embeddings.word_embeddings(input_ids)
        inputs_embeds = inputs_embeds + question_emb # Adding Quesiton Emb at boq token
        ...
        outputs = self.bert(inputs_embeds=inputs_embeds, token_type_ids = token_type_ids , attention_mask=extra_mask)
