...
class XLNetForQuestionGenerationFluidSegments(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(XLNetForQuestionGenerationFluidSegments, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
...
class XLNetForQuestionGeneration(XLNetPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    def __init__(self, config):
        super(XLNetForQuestionGeneration, self).__init__(config)
        #self.bert = BertModel(config)
        ...
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.config = config
        #self.cls = BertPreTrainingHeads(config, self.transformer.embeddings.word_embeddings.weight)
        self.qgen_outputs = nn.Linear(config.hidden_size, config.vocab_size)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        #sequence_output, pooled_output, attns = self.transformer(input_ids=input_ids, attention_mask=extra_mask, token_type_ids=token_type_ids)
        outputs = self.transformer(input_ids=input_ids, token_type_ids = token_type_ids) 
