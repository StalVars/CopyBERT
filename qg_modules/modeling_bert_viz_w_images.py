...
#from qg_modules.modeling_layoutlmv3 import LayoutLMv3PatchEmbeddings
from transformers.models.layoutlmv3.modeling_layoutlmv3  import LayoutLMv3PatchEmbeddings

from transformers.file_utils import PYTORCH_PRETRAINED_BERT_CACHE, WEIGHTS_NAME, CONFIG_NAME

from qg_modules.bert_model import BertModel
...
class BertPreTrainedModel_own(nn.Module):
...
class BertModel_own(BertPreTrainedModel):
...
class BertModel_prev(BertPreTrainedModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
...
class BertForPhraseExtraction(BertPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertForPhraseExtraction, self).__init__(config)
        self.bert = BertModel(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            sequence_output2, _ = self.bert(input_ids, position_vec, attention_mask, output_all_encoded_layers=False)
...
class BertForQuestionGenerationFluidSegments(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(BertForQuestionGenerationFluidSegments, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        #sequence_output, pooled_output, attns = self.bert(input_ids, token_type_ids + answer_phrase_ids, extra_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        outputs = self.bert(input_ids, token_type_ids + answer_phrase_ids, extra_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
...
class BertModelWithViz(BertModel):
...
class TwoDPosEmbeddings(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    def __init__(self, config):
        ...
        #self.bert = BertModelWithViz(config)
        self.bert = BertModel(config)
...
class BertForQuestionGeneration(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(BertForQuestionGeneration, self).__init__(config)
        ...
        self.layoutlmv3_config = AutoConfig.from_pretrained("/netscratch/svaranasi/raid-9206/saved_bert_models/saved-microsoft_by_layoutlmv3-base/")

        size = int(self.layoutlmv3_config.input_size / self.layoutlmv3_config.patch_size)

        self.layoutlmv3_cls_token = nn.Parameter(torch.zeros(1, 1, self.layoutlmv3_config.hidden_size))
        self.layoutlmv3_pos_embed = nn.Parameter(torch.zeros(1, size * size + 1, self.layoutlmv3_config.hidden_size))
        self.layoutlmv3_pos_drop = nn.Dropout(p=0.0)
        self.norm = nn.LayerNorm(self.layoutlmv3_config.hidden_size, eps=1e-6)


        self.patch_embed = LayoutLMv3PatchEmbeddings(self.layoutlmv3_config)
    ...
    def forward_image(self, pixel_values):
        ...
        cls_tokens = self.layoutlmv3_cls_token.expand(batch_size, -1, -1)
        embeddings = torch.cat((cls_tokens, embeddings), dim=1)

        # add position embeddings
        if self.layoutlmv3_pos_embed is not None:
            embeddings = embeddings + self.layoutlmv3_pos_embed

        embeddings = self.layoutlmv3_pos_drop(embeddings)
    ...
    def get_copy_probs(self,sequence_output, self_attentions,src_seq,cls_att=0,answer_ids=None,maxout_pointer=False,para_mask=None):
        ...
        # Remove visual attention:
        attention = attention[:, 197:, 197:] # size batch_size x 384 x 384
    ...
    def forward(self, 
        ...
        vis_mask_ids = torch.ones(input_mask_ids.size(0), 197, 1).to(input_ids.device) # 197 is vis embeddings size
        new_input_mask_ids = torch.cat([vis_mask_ids, input_mask_ids], dim=1)

        prev_question_mask_ids = question_mask_ids 
        question_mask_ids = torch.cat( [ torch.zeros_like(vis_mask_ids), 
                                         question_mask_ids], dim=1)
        new_input_ids = torch.cat( [ torch.zeros_like(vis_mask_ids), 
                                         input_ids.unsqueeze(-1)], dim=1)

        input_mask_ids = new_input_mask_ids 

        #Mask begin of question
        boq_mask = (input_ids != self.MASKID).unsqueeze(-1).float()
        boq_mask = torch.cat( [ vis_mask_ids,
                                         boq_mask], dim=1)
        question_mask_ids = question_mask_ids * boq_mask
        #print(question_mask_ids.size(), question_mask_ids.sum(1))
        #torch.save(question_mask_ids,"question_mask_ids")
        question_mask = torch.bmm(question_mask_ids, question_mask_ids.transpose(1,2)) * torch.tril(torch.ones(question_mask_ids.size(1),question_mask_ids.size(1))).unsqueeze(0).cuda() 

        # For Question Generation use below line
        prediction_ids = (input_ids.unsqueeze(-1) * prev_question_mask_ids.long()).squeeze(-1)
        #prediction_ids = (new_input_ids * question_mask_ids.long()).squeeze(-1)
        # For simple LM use below line
        #prediction_ids = input_ids
        prediction_ids[:,0]=0 #self.PADID
        prediction_ids = torch.cat((prediction_ids[:,1:],prediction_ids[:,0:1]),-1) # batch_size x seq_length
        ...
        # For Question Generation
        #sequence_output, pooled_output, attns = self.bert(input_ids=input_ids, token_type_ids = token_type_ids + answer_phrase_ids, attention_mask=extra_mask)
        #bboxes=bboxes,
        ...
        # pixel_embeddings
        patch_height, patch_width = int(pixel_values.shape[2] / self.layoutlmv3_config.patch_size), int(
                pixel_values.shape[3] / self.layoutlmv3_config.patch_size
            )

        batch_size = input_ids.size(0)
        visual_embeddings = self.forward_image(pixel_values)
        visual_attention_mask = torch.ones(
                (batch_size, visual_embeddings.shape[1]), dtype=torch.long, device=input_ids.device
            )

        #print("extra_mask:", extra_mask.size())
        #print("visual_attention_mask:", visual_attention_mask.size())
        #print("embeds size:", visual_embeddings.size(), new_inputs_embeds.size())

        #new_inputs_embeds  = torch.cat([new_inputs_embeds, visual_embeddings], dim=1)
        new_inputs_embeds  = torch.cat([visual_embeddings, new_inputs_embeds], dim=1)
        token_type_ids = token_type_ids + answer_phrase_ids
        vis_token_type_ids = torch.zeros_like(visual_attention_mask)
        new_token_type_ids = torch.cat([vis_token_type_ids, token_type_ids], dim=-1)

        #print("concat embeds size:", new_inputs_embeds.size()) 
        ...
        #q_outputs =  self.bert(input_ids = query_ids, token_type_ids=torch.zeros_like(query_ids), attention_mask=query_input_mask)
        # For LM
        #sequence_output, pooled_output, attns = self.bert(input_ids, token_type_ids, extra_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        ...
        if copymethod > 0:
          ...
          if copymethod == 1:
            ...
            p = p[:, 197:]
            copy_qgen_logits = (1-p)* copy_qgen_logits 
          ...
          qgen_logits = qgen_logits[:,197:,:] + copy_qgen_logits
        else:
          #print("no copy method")
          qgen_logits =  self.cls.predictions(sequence_output)
        ...
        if not evaluate: #start_positions is not None and end_positions is not None:
         ...
         #prediction_ids = prediction_ids[:, 197:, :] # remove prediction ids for visual positions
         #print("pred size, qgen_logits size:", prediction_ids.size(), qgen_logits.size())
         loss = loss_fct(qgen_logits.view(-1,self.vocab_size), prediction_ids.view(-1))
        ...
        if start_positions is not None and end_positions is not None:
            ...
            return start_logits, end_logits
        '''
class BertForQuestionLM(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(BertForQuestionLM, self).__init__(config)
        #self.bert = BertModelWithViz(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertEncoder2Decoder(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert_decoder = BertDecoderModel(config)
    ...
    def forward(self, input_ids, output_ids, input_token_type_ids=None, output_token_type_ids=None, input_mask=None, output_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False,copymethod=1):
        ...
        encoder_output, pooled_encoder_output, self_attns = self.bert_encoder(input_ids, input_token_type_ids + answer_phrase_ids, input_mask, output_all_encoded_layers=False,decoder_mask=decoder_mask,attn=True)
        ...
        decoder_output, pooled_decoder_output, attns = self.bert_decoder(output_ids, encoder_output, output_mask, dec2enc_mask, output_token_type_ids, output_all_decoded_layers=False,attn=True)
