...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    ...
    def semi_diagnol_mask(self, token_type_ids, attention_mask,visual_attention_mask):

               
        only_input = (attention_mask - token_type_ids) 

        only_input = torch.cat([only_input, visual_attention_mask], dim=1) # ocr_text + visual_tokens
        input_w_q = torch.cat([attention_mask, visual_attention_mask], dim=1) # ocr_text + question_text + visual_tokens
        q_mask = torch.cat([token_type_ids, torch.zeros_like(visual_attention_mask)], dim=1) # question_text




        only_input = only_input.unsqueeze(-1).float() # b x seq x 1
        input_w_q = input_w_q.unsqueeze(-1).float()
        q_mask = q_mask.unsqueeze(-1).float()


        only_input_2d = only_input.matmul(only_input.transpose(-1,-2)) # b x seq x seq 
        attention_4_question = q_mask.matmul(only_input.transpose(-1,-2))
        question_diag = q_mask.matmul(q_mask.transpose(-1,-2)) 
        question_diag = torch.tril(question_diag)

        '''
        print("token_type_ids:sum(-2):",only_input_2d[0].sum(-2))
        print("only_input_2d:sum(-1):",only_input_2d[0].sum(-1))
        print("only_input_2d:sum(-2):", only_input_2d[0].sum(-2))
        print("question_diag:sum(-1):",question_diag[0].sum(-1))
        print("question_diag:sum(-2):",question_diag[0].sum(-2))
        print("attn4question:sum(-1)",attention_4_question[0].sum(-1))
        print("attn4question:sum(-2)",attention_4_question[0].sum(-2))
        input("-")
        '''

        mask = torch.logical_or(only_input_2d, question_diag)

        #print("mask sum:", mask.sum(-1)[0])
        #print("mask sum:", mask.sum(-2)[0])
        #print("attn4q sum:", attention_4_question.sum(-1).sum(-1))

        mask = torch.logical_or(mask, attention_4_question)

        #print("mask sum after logical or on attn4question:")
        #print("mask sum:", mask.sum(-1)[0])
        #print("mask sum:", mask.sum(-2)[0])

        # debug - 2-D mask with full attention
        # mask = input_w_q.matmul(input_w_q.transpose(-1,-2))
    ...
    def forward(
        ...
        if input_ids is not None or inputs_embeds is not None:
            ...
            embedding_output = self.embeddings(
                ...
                token_type_ids=torch.zeros(input_shape, dtype=torch.long, device=device), #token_type_ids,
        ...
        if pixel_values is not None:
            ...
            if attention_mask is not None:
                ...
                #print("attention mask shape:", attention_mask.shape, attention_mask.size())
                #print("attn sum:", attention_mask.sum(-1))
                if len(attention_mask.shape) == 3:
                  diag_attention_mask = attention_mask
                else:
                 diag_attention_mask = self.semi_diagnol_mask(token_type_ids, attention_mask, visual_attention_mask)
                #print("attention_mask sum:", attention_mask[0].sum(-1))
                #print("visualmask sum:", visual_attention_mask[0].sum(-1))

                #print(diag_attention_mask[0].size(), diag_attention_mask[0].sum(-1), diag_attention_mask[0].sum(-2))
                #input("-diag attention mask")

                diag_attention_mask = diag_attention_mask.to(dtype=self.dtype)
                extended_attention_mask=diag_attention_mask.unsqueeze(1)
            else:
            ...
            if self.config.has_relative_attention_bias or self.config.has_spatial_attention_bias:
                if self.config.has_spatial_attention_bias:
                    ...
                    if bbox is not None:
                        ...
                        #Reversed
                        final_bbox = torch.cat([visual_bbox, bbox], dim=1)
                ...
                if input_ids is not None or inputs_embeds is not None:
                    ...
                    # Reversed
                    final_position_ids = torch.cat([visual_position_ids, position_ids], dim=1)
                else:
        ...
        ''' calculate semi-diagnol mask including visual tokens'''
        #print("calculating semi-diagnol mask with visual tokens")

        #Debug lines
        #print("extended mask: 1", extended_attention_mask.sum(-2))
        #print("extended mask: 2", extended_attention_mask.sum(-1))
        #extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(diag_attention_mask, input_shape, device)
        ...
        #torch.save(extended_attention_mask[0], "attn_example.pt")
        #input("-")
