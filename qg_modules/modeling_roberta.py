...
#from transformers.modeling_bert import BertPreTrainedModel, BertEmbeddings
...
class RobertaForQuestionGenerationFluidSegments(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(RobertaForQuestionGenerationFluidSegments, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        input_mask_ids = (input_mask!=0).unsqueeze(-1).float()
...
class RobertaForQuestionGeneration(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    def __init__(self, config):
        super(RobertaForQuestionGeneration, self).__init__(config)
        #self.bert = BertModel(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        sequence_output, pooled_output, attns = self.roberta(input_ids=input_ids, attention_mask=extra_mask)
