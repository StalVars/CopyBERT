...
class BertForDAGInduction(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, num_labels):
        super(BertForDAGInduction, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                # this probably is the latent permutation loss which uses doubly stochastic matrix and gumbel noise to sample discrete permutations for further step
                kl_div = self.latent_permutation_loss(self_attns.transpose(0,1),src_lengths)
    ...
    def latent_permutation_loss(self,X,lengths):
        # KL divergence between X/temp and eps/temp_prior
        # eps = gumble_noise(X)
        ...
        '''
        S = 0
        r= self.sinkhorn_temp_prior/self.sinkhorn_temp
        gamma_r = math.gamma(1+r)
        '''
        if torch.exp(-X*r).sum() > 300:
         print(X.min())
         input("X")
         print("ratio r",r)
         print("S1",r/X.sum())
         print("S2",torch.exp(-X*r).sum())
         print("Size",X.size())
         print("Gamma",gamma_r)
         input("checks")
        '''
        masks = torch.zeros(X.size()).cuda()
        for i,l in enumerate(lengths):
          masks[:l+1,i,:l+1] = 1 #masks[:l,i,:l] = 1
        masks[:,:,0] = 0 # to SOS is impossible

        for i in range(masks.size(0)):
            masks[i,:,i]=0 # word-to-word itself transition is impossible
        #masks = Variable(masks,requires_grad=False).type_as(X)
        X=X*masks
        ...
        for i,l in enumerate(lengths):
             #S = S + r / X[:l,i,:l].sum()+gamma_r*torch.exp( -X[:l,i,:l]*r).sum()
             #S = S + r * (X[:l+1,i,:l+1]*masks[:l+1,i,:l+1]).sum()+gamma_r*(torch.exp( -X[:l+1,i,:l+1]*r)*masks[:l+1,i,:l+1]).sum() + math.log(1/r)*(l+1)**2
             S = S + r / (X[:l+1,i,:l+1]*masks[:l+1,i,:l+1]).sum()+gamma_r*(torch.exp( -X[:l+1,i,:l+1]*r)*masks[:l+1,i,:l+1]).sum() #+ math.log(1/r)*(l+1)**2

        #S = r*X.sum() + gamma_r*torch.exp(-X*r).sum()
        #S = r/X.sum() + gamma_r*torch.exp(-X*r).sum()
        #S = r*X.sum() + gamma_r*torch.exp(-X*r).sum() + (X.size(1)*X.size(2))*math.log(1/r)
        #S = S / X.size(0) # Average batch_size
    ...
    def gumbel_noise_sample(self,input,temperature = 1):
...
class BertForQuestionGeneration(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False, copymethod=1):
        ...
        extra_mask = extra_mask * torch.tril(torch.zeros_like(extra_mask) + 1 )
        ...
        # token_type_ids should contain Answer phrase
        ...
        if copymethod > 0:
          ...
          #qgen_logits = p * self.qgen_outputs(sequence_output,pooled_output)
          qgen_logits, _ =  self.qgen_outputs(sequence_output,pooled_output)
          #last_hidden = new_encoded_layers[-1][:, -1:, :] #b x 1 x hidden_size
          ...
          if copymethod == 1:
            ...
            copy_qgen_logits = (1-p) * self.get_copy_probs2(sequence_output,para_mask,input_ids)
          qgen_logits = qgen_logits + copy_qgen_logits
        else:
          #print("no copy method")
          qgen_logits =  self.qgen_outputs(sequence_output,pooled_output)
...
class BertForQuestionLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.iscuda = True
        self.attentionsquare = nn.Linear(config.hidden_size, config.n_heads * config.n_layers ) # config.n_heads * config.n_layers
        self.linear_copy_probs = nn.Linear(config.hidden_size, config.hidden_size ) # config.n_heads * config.n_layers
...
class BertEncoder2Decoder(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.linear_copy_probs = nn.Linear(config.hidden_size, config.hidden_size ) # config.n_heads * config.n_layers
        self.combine_input_layers = nn.Linear(config.hidden_size, 12 ) # to config.n_layers


    @classmethod
    def from_pretrained_initial (cls, pretrained_model_name_or_path_1, pretrained_model_name_or_path_2, *inputs, **kwargs):
    ...
    def encode_step(self, input_ids,input_mask, input_token_type_ids, start_positions,end_positions):
        ...
        encoder_output, pooled_encoder_output, self_attns = self.bert_encoder(input_ids, input_token_type_ids + answer_phrase_ids, input_mask, 
                output_all_encoded_layers=True,decoder_mask=True,attn=True)

        last_output = encoder_output[-1]
        encoder_output = [a.unsqueeze(1) for a in encoder_output]
        encoder_output = torch.cat(encoder_output,dim=1) # batch_size x layer_size(12) x seq_len x hidden_size
        encoder_output = encoder_output.transpose(1,2) # batch_size x seq_len x layer_size x hidden_size
        #self_attns = torch.cat(self_attns,dim=1).transpose(1,2) # batch_size x seq_len x 12*12 x seq_len
        attention_layers = self.combine_input_layers(last_output).unsqueeze(2) # batch_size x seq_len x 1 x 12 
        informed_encoder_output = torch.matmul(attention_layers, encoder_output).squeeze(2) # batch_size x seq_len x  hidden_size
        

        return informed_encoder_output, pooled_encoder_output, self_attns

    def decode_step(self,output_ids, encoder_output, output_mask,  output_token_type_ids, input_ids, copymethod=1):
    ...
    def forward(self, input_ids, output_ids, input_token_type_ids=None, output_token_type_ids=None, input_mask=None, output_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False,copymethod=1):
       ...
       encoder_output, _, _ = self.encode_step(input_ids,input_mask, input_token_type_ids, start_positions,end_positions)

       prediction_ids = output_ids
       prediction_ids[:,0]=self.PADID
       prediction_ids = torch.cat((prediction_ids[:,1:],prediction_ids[:,0:1]),-1) # batch_size x seq_length

       qgen_logits = self.decode_step(output_ids, encoder_output, output_mask, output_token_type_ids, input_ids, copymethod=copymethod)
    ...
    def forward_prev(self, input_ids, output_ids, input_token_type_ids=None, output_token_type_ids=None, input_mask=None, output_mask=None, start_positions=None, end_positions=None,decoder_mask=True,evaluate=False,copymethod=1):
