...
def convert_examples_to_features_4gen(examples, tokenizer, max_seq_length,
...
def translate_batch(
            ...
            n_best=1,
            beam_size=10,
            return_attention=False,split=False,device=0):
        ...
        beam = BeamSearch(
            ...
            block_ngram_repeat=True,bos=cls_id, eos=sep_id,device=device)
...
def multi_generate(model, tokenizer, args, devices, no_of_datapoints=-1,train=False,qlm_model=None,test_examples=None):
   ...
   for m in range(len(devices)):
     gen = thread.start_new_thread( evaluate_by_generate, (model, tokenizer)) #, args, device, train=False,qlm_model=None,test_examples=test_examples_s[m]))

def evaluate_by_generate(model, tokenizer, args, device,no_of_datapoints=-1,train=False,qlm_model=None,test_examples=None,tag=0):

        #pred_file = open(str(args.output_dir)+"/preds."+filetag+"."+args.tag+".txt","w")
        #context_file = open(str(args.output_dir)+"/contexts."+filetag+"."+args.tag+".txt","w")
        pred_file = open(str(args.output_dir)+"/preds."+"."+args.tag+".txt","w")
        context_file = open(str(args.output_dir)+"/contexts."+"."+args.tag+".txt","w")
        ...
        eval_features = convert_examples_to_features_4gen(
        ...
        log_file.write("Num orig examples = "+str(len(eval_test_examples))+"\n")
        log_file.flush()
        ...
        with torch.no_grad():
         for index, f in enumerate(tqdm(eval_features, desc="Evaluating", disable=args.local_rank not in [-1, 0])):
         #for input_ids, input_mask, segment_ids, start_positions, end_positions, example_indices in tqdm(eval_features, desc="Evaluating", disable=args.local_rank not in [-1, 0]):
            input_ids = torch.tensor([f.input_ids])
            input_mask = torch.tensor([f.input_mask])
            segment_ids = torch.tensor([f.segment_ids])
            start_positions = torch.tensor([f.start_position])
            end_positions = torch.tensor([f.end_position])
            tokens = f.tokens
            if len(all_results) % 1000 == 0:
                logger.info("Processing example: %d" % (len(all_results)))
  
            if index < args.genfrom:
                continue
            if index > args.gento:
                continue
            ...
            references = [ re.sub("[     ]*\[SEP\]","",re.sub(".*\[MASK\]",""," ".join(tokens))) ]
            ...
            gen_tup = translate_batch(model, tokenizer, inputs, 20, args,qamodel, beam_size=args.beam_size, qlm_model=qlm_model,device=device)
            ...
            if args.rerank:
              ...
              new_start_indices = torch.zeros(len(preds[0]),device=device).long() + start_positions - 1 # -1 because [CLS] token is removed in the preds
              new_end_indices = torch.zeros(len(preds[0]),device=device).long() + end_positions - 1 # -1 because [CLS] token is removed in the preds
            ...
            context_file.write(references[0]+"\n")
            pred_file.flush()
            context_file.flush()
...
def main():
    ...
    parser.add_argument("--tok_model", default=None, type=str, required=True,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                        "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, "
                        "bert-base-multilingual-cased, bert-base-chinese.")
    ...
    parser.add_argument("--gento", default=9999999, type=int,
    ...
    log_file = open("qg_logs/log."+re.sub("/","_",str(args.output_dir))+filetag+".txt","w")
    filetag = "_beam_"+str(args.beam_size)+"_nbest_"+str(args.n_best_size)+"_rerank_"+str(args.rerank)+"_pmi_"+pmi+"_cp_"+str(args.copymethod)+"."+filetag
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
      ...
      if args.no_cuda:
       devices=[ device for i in range(2) ]
       eval_dev_examples = read_squad_examples(
            input_file=data_file, is_training=False, version_2_with_negative=args.version_2_with_negative)
       multi_generate(model, tokenizer, args,devices,qlm_model=qlm_model,test_examples=eval_dev_examples)
