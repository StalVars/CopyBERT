...
def evaluate_by_loss(model, tokenizer, args, device,no_of_datapoints=-1,train=False):
    ...
    cache_file = args.predict_file+'_{0}_{1}_{2}_{3}'.format(
             model_name, str(args.max_seq_length), str(args.doc_stride), str(args.max_query_length))
    cache_file = data_dir +"/cache/"+cache_file
     ...
     for eval_features in  convert_examples_to_features(
        ...
        with torch.no_grad():
         for input_ids, input_mask, segment_ids, start_positions, end_positions, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=args.local_rank not in [-1, 0]):
            ...
            best_pred_text = re.sub(" ##","",best_pred_text) # Remove BERT spacing
            reference = re.sub(" ##","",references[0]) # Remove BERT spacing
...
def main():
    ...
    if args.do_train:
     ...
     for ep in trange(int(args.num_train_epochs), desc="Epoch"):
       ...
       cache_file = args.train_file+'_{0}_{1}_{2}_{3}'.format(
             model_name, str(args.max_seq_length), str(args.doc_stride), str(args.max_query_length))
       cache_file = data_dir +"/cache/"+cache_file
