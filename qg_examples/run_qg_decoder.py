...
#from transformers import DPRQuestionEncoder as QEncoder
from qg_modules.hf_bert import HFBertEncoder as QEncoder 
...
def translate_batch(
        ...
        input_ids, segment_ids, input_mask, query_ids, query_input_mask, references = batch
        ...
        # Encode question
        question_outputs = encoder_model(query_ids, torch.zeros_like(query_ids), query_input_mask)
        question_emb = question_outputs[1] # pooled output

        log_probs = model(input_ids, segment_ids, input_mask, question_emb=question_emb,copymethod=args.copymethod,evaluate=True)
        ...
        #dummy start/end of answer phrase since not used
        start_positions = torch.zeros(input_ids.size(0)).to(input_ids.device)
        ...
        with torch.no_grad():
          for step in range(max_length):
            ...
            if split:
              ...
              log_probs1 = model(input_ids[:5], segment_ids[:5], input_mask[:5], question_emb=question_emb ,copymethod=args.copymethod,evaluate=True)
              log_probs2 = model(input_ids[5:], segment_ids[5:], input_mask[5:], question_emb ,copymethod=args.copymethod,evaluate=True)
...
def evaluate_by_loss(model, encoder_model, tokenizer, args, device,no_of_datapoints=-1,train=False):
        ...
        eval_features = convert_examples_to_features_qg(
        ...
        all_query_mask = torch.tensor([f.query_input_mask for f in eval_features], dtype=torch.long)

        #all_tokens = [f.tokens for f in eval_features]
        all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_query_ids, all_query_mask, all_example_index)
        ...
        no_of_examples=0
        with torch.no_grad():
          for input_ids, input_mask, segment_ids, query_ids, query_input_mask, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=args.local_rank not in [-1, 0]):
            ...
            loss = model(input_ids, segment_ids, input_mask, question_emb = question_emb, copymethod=args.copymethod)
            eval_loss += loss 
...
def evaluate_by_generate(model, tokenizer, args, device,no_of_datapoints=-1,train=False,encoder_model=None,test_examples=None):
        ...
        with torch.no_grad():
         for input_ids, input_mask, segment_ids, query_ids, query_input_mask, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=args.local_rank not in [-1, 0]):
            ...
            #references = [ re.sub("[     ]*\[SEP\]","",re.sub(".*\[QUESTION\]",""," ".join(all_tokens[i]))) for i in example_indices ]
            references = [ " ".join(tokenizer.convert_ids_to_tokens([i.item() for i in query_ids[0]] )) ] 
            #references = [ re.sub(".*\[MASK\]",""," ".join(all_tokens[i])) for i in example_indices ]
            inputs = [ input_ids, segment_ids,input_mask, query_ids, query_input_mask, references ]
            ...
            gen_tup = translate_batch(model, tokenizer, inputs, 20, args,qamodel, beam_size=args.beam_size, encoder_model=encoder_model)
            ...
            best_pred_text, best_score = print_beam(tokenizer, preds[0], segments[0],scores=scores[0],start_indices = None, end_indices=None, secondary_scores=None)
...
def main():
    ...
    parser.add_argument("--encoder_model", default=None, type=str, required=True,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                        "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, "
                        "bert-base-multilingual-cased, bert-base-chinese.")
    ...
    parser.add_argument("--beam_size", default=5, type=int,
                        help="The beam width")
    parser.add_argument("--copymethod", default=0, type=int,
    ...
    #question_id = tokenizer.convert_tokens_to_ids(["[QUESTION]"])[0]
    ...
    cls_id,sep_id = tokenizer.convert_tokens_to_ids([cls_token,sep_token])
    model.boq_id = sep_id # boq is same as eoq == sep_token
    encoder_model = QEncoder.from_pretrained(args.encoder_model) 

    # Resize the token embeddings
    #model.resize_token_embeddings(len(tokenizer)) # if you add special tokens
    ...
    if args.do_train:
        ...
        try:
            ...
            #print("except")
            train_features = convert_examples_to_features_qg(
        ...
        all_query_mask = torch.tensor([f.query_input_mask for f in train_features], dtype=torch.long)
        ...
        #all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_query_ids, all_query_mask) 
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
                ...
                input_ids, input_mask, segment_ids, query_ids, query_input_mask  = batch
             ...
             and  (s >= start_positions.item() and s <= end_positions.item()) ])) for b in range(input_ids.size(0))  ]
                ...
                if (step+1) % args.training_sanity_check_steps  == 0:
                   if args.gen_during_train:
                     evaluate_by_generate(model, tokenizer, args,device, encoder_model = encoder_model, no_of_datapoints=10,train=False)
                   eval_loss = evaluate_by_loss(model, encoder_model, tokenizer, args,device)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
      ...
      evaluate_by_generate(model, tokenizer, args,device,encoder_model = encoder_model) 
