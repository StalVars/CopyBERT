...
from qg_modules.modeling import BertForQuestionGeneration
from qg_modules.optimization import BertAdam, WarmupLinearSchedule
...
def main():
    ...
    if args.fp16:
        ...
        '''
        print("opt steps",num_train_optimization_steps)
        input("")
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
             ...
             and  (s >= start_positions.item() and s <= end_positions.item()) ])) for b in range(input_ids.size(0))  ]
                ...
                if (step ) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    if step % 1000 == 0:
                     log_file.write("step="+str(step)+"	lr="+str(optimizer)+"\n")
                     log_file.flush()
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        tokenizer = BertTokenizer.from_pretrained(args.model_name_or_path, do_lower_case=args.do_lower_case)#, do_basic_tokenize=False)
