...
from qg_modules.modeling import TransformersForQuestionGeneration
...
def main():
    ...
    # Prepare model
    model = TransformersForQuestionGeneration.from_pretrained(args.model_name_or_path,
             ...
             and  (s >= start_positions.item() and s <= end_positions.item()) ])) for b in range(input_ids.size(0))  ]
                ...
                if (step +1) % args.training_sanity_check_steps  == 0:
                   ...
                   if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and better_eval:
                       ...
                       model = TransformersForQuestionGeneration.from_pretrained(args.output_dir)  # , force_download=True)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = TransformersForQuestionGeneration.from_pretrained(args.output_dir)
        ...
        #input(i"")
        model = TransformersForQuestionGeneration.from_pretrained(args.model_name_or_path) #(,do_lower_case=args.do_lower_case)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
      if args.qlm_model:
        qlm_model = BertForQuestionLM.from_pretrained(args.qlm_model,
                cache_dir=os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE), 'distributed_{}'.format(args.local_rank)))
        #qlm_model = TransformersForQuestionGeneration.from_pretrained(args.qlm_model,
        #        cache_dir=os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE), 'distributed_{}'.format(args.local_rank)))
        qlm_model.cuda()
      else:
        qlm_model=None
