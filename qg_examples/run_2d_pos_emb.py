...
from pytorch_pretrained_bert_local.modeling import TwoDPosEmbeddings, BertConfig
...
#from qg_modules.modeling_bert import TwoDPosEmbeddings
from qg_modules.modeling_bert_viz import TwoDPosEmbeddings
...
MODEL_CLASSES = {
    "bert": (BertConfig, TwoDPosEmbeddings, BertTokenizer, "[CLS]","[MASK]","[SEP]"),
    "span": (BertConfig, TwoDPosEmbeddings, BertTokenizer, "[CLS]","[MASK]","[SEP]"),
...
'''
MODEL_CLASSES = {
    "bert": (BertConfig, TwoDPosEmbeddings, BertTokenizer),
    ...
    "distilbert": (DistilBertConfig, DistilTwoDPosEmbeddings, DistilBertTokenizer),
...
def translate_batch(
        ...
        log_probs = model(input_ids, segment_ids, input_mask, start_positions, end_positions, bboxes=bboxes, copymethod=args.copymethod,evaluate=True)
...
def evaluate_by_loss(model, tokenizer, args, device,no_of_datapoints=-1,train=False):
    ...
    with torch.no_grad():
     for  step in range(3000):
        points = [ get_random_points() for i in range(args.predict_batch_size)]
        labels = []
        for xi in range(len(points)):
           row_labels = []
           for yi in range(len(points)):
               row_labels.append(l2_distance(points[xi], points[yi]))
           labels.append(row_labels)

        labels = torch.FloatTensor(labels).to(device)
        input_points = torch.LongTensor(points).to(device)
        ...
        if True:
            ...
            loss,cosine_sim = model(input_points, 
                    labels)
            eval_loss += loss 
...
def evaluate_by_generate(model, tokenizer, args, device,no_of_datapoints=-1,train=False,qlm_model=None,test_examples=None):
     ...
     for eval_features in  convert_examples_to_features(
        ...
        logger.info("  Num %s split features = %d,",(evi,   len(eval_features)) )
...
def main():
    ...
    if args.fp16:
        ...
        num_of_optimization_steps = 10000 * args.num_train_epochs
       ...
       for train_features in convert_examples_to_features(
        ...
        if True:
            ...
            for i in range(100000):
                points = [ get_random_points() for i in range(args.train_batch_size)]
                labels = []
                for xi in range(len(points)):
                   row_labels = []
                   for yi in range(len(points)):
             ...
             and  (s >= start_positions.item() and s <= end_positions.item()) ])) for b in range(input_ids.size(0))  ]
                ...
                loss, cosine_sim = model(input_points,labels)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
      if args.qlm_model:
        qlm_model = BertForQuestionLM.from_pretrained(args.qlm_model,
                cache_dir=os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE), 'distributed_{}'.format(args.local_rank)))
        #qlm_model = TwoDPosEmbeddings.from_pretrained(args.qlm_model,
        #        cache_dir=os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE), 'distributed_{}'.format(args.local_rank)))
        qlm_model.cuda()
      else:
        qlm_model=None
